document.getElementById('addButton').addEventListener('click', () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      fetch('https://api-planetshop.online/wall-api/addNewGroup', { // Замените URL на ваш серверный
        method: 'POST',
        headers: {
      ...
      .then((data) => {
        ...
        if (!data.status) {
            document.getElementById('result').textContent = 'Группа добавлена!';
        } else {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function sendUrlToServer() {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
          ...
          fetch('https://api-planetshop.online/wall-api/checkGroup', { // Замените URL на ваш серверный
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },  
          ...
          .then(data => {
              if (!data.exist) {
                  ...
                  document.getElementById('result').textContent = 'Уже добавлена!';
              }
          })
          .catch(()=> {
            document.getElementById('result').textContent = 'Ошибка';
  ...
  function getData(currentTabUrl) {
    ...
    if (currentTabUrl.includes('wall')) {
      ...
      if (match) {
        ...
        document.getElementById('result').textContent = `Не удалось найти соответствие`;
        return;
      }
    } else {
